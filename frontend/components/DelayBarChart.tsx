...
export default function DelayBarChart({ data }: Props) {
  const rows = [...data]
    ...
    .slice(0, 10)
    .map((d) => ({ stop: d.stop_id, delay: Math.round(d.avg_delay_mins * 10) / 10 }));

  if (rows.length === 0) {
    return <p className="text-sm text-slate-500">No delay data yet.</p>;
  }
    ...
    <div className="h-80 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={rows} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
          ...
          <XAxis dataKey="stop" tick={{ fontSize: 11 }} />
