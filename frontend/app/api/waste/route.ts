...
export async function GET(req: NextRequest) {
  const kind = req.nextUrl.searchParams.get("kind") ?? "schedule";
  const neighbourhoodId = req.nextUrl.searchParams.get("neighbourhood_id");
  let path = "/api/waste/schedule";
  if (kind === "diversion") {
    path = "/api/waste/diversion-rate";
  } else if (neighbourhoodId) {
    path = `/api/waste/schedule?neighbourhood_id=${neighbourhoodId}`;
  }
  try {
